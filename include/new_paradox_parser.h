...
#ifndef NEW_PARADOX_PARSER_H
#define NEW_PARADOX_PARSER_H
...
namespace wiz {
	...
	public:
		...
		bool operator() (VECTOR<Token2>*, const wiz::LoadDataOption&, int, char*&)
		{
			count = 1;
			return false;
		}
	...
	namespace load_data {
		...
		public:
			...
			template <class Reserver>
			static bool _LoadData3_2(VECTOR<Token2>& strVec, Reserver& reserver, UserType& global, const wiz::LoadDataOption& option, const int pivot_num, const int lex_thr_num) // first, strVec.empty() must be true!!
			{
				char left[] = "{";
				char right[] = "}";
		...
		public:
			static bool LoadDataFromFile(const std::string& fileName, UserType& global, int lex_thr_num, int parse_thr_num) /// global should be empty
			{
				...
				try {
					...
					option.LineComment.push_back("#"); // always #! - now... - todo!

					ifReserver.Num = 1 << 19;
					//	strVec.reserve(ifReserver.Num);
					// cf) empty file..
					if (false == _LoadData3_2(strVec, ifReserver, globalTemp, option, parse_thr_num - 1, lex_thr_num))
					{
						inFile.close();
